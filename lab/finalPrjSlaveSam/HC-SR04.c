...
//    TIMER_A0 -> CCTL[2] = 0xE0;  //reset / set
    TIMER_A2 -> CCTL[1] = 0xE0;  //reset / set


//    TIMER_A0 -> EX0 = 0; //divider expansion  /8*(8) 1Hz signal =~ 187500 counts

    TIMER_A2 -> CCR[0] = TriggerPinPeriodVal; //set the period of CCR
    //timerA initialization for Echo pin
    TIMER_A2->CTL = 0X2D4;      // SMCLK, Up Mode, clear TAR to start, DIV 8
//    TIMER_A2 -> CTL = 0b0000001011100010; //SMCLK, continuous mode, 8 div, clear.
    TIMER_A2-> CCTL[2] = 0b1100100100010100;  //Capture on rising / falling edge,
    TIMER_A2 -> EX0 = 3; //divider expansion  /8*(8) 1Hz signal =~ 187500 counts
    TIMER_A2-> R = 0;  //reset count value
}

uint16_t xHCSR04_GetSonarValue()
{
    if(TIMER_A2->CCTL[2] & BIT3)
    {
        TIMER_A2->R = 0;
    }
    else if(TIMER_A2->CCTL[2] &~ BIT3)
    {
